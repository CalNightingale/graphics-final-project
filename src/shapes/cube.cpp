...
void Cube::makeFace(glm::vec3 topLeft,
    ...
    glm::vec3 leftRightTileVec = (topRight - topLeft) / (float)m_param1;
    glm::vec3 topDownTileVec = (bottomRight - topRight) / (float)m_param1;
    for (int i = 0; i < m_param1; i++) {
        for (int j = 0; j < m_param1; j++) {
            glm::vec3 topLeftTile = topLeft + (float)i*leftRightTileVec + (float)j*topDownTileVec;
            glm::vec3 topRightTile = topLeftTile + leftRightTileVec;
            glm::vec3 bottomLeftTile = topLeftTile + topDownTileVec;
            glm::vec3 bottomRightTile = topLeftTile + leftRightTileVec + topDownTileVec;
